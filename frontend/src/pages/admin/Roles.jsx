...
function Toast({ message, type = 'success', onClose }) {
    ...
    return (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-xl shadow-2xl text-slate-900 text-sm font-medium animate-fade-in ${type === 'success' ? 'bg-emerald-600' : 'bg-red-600'}`}>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5 flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full text-slate-900" style={{ background: role.color }}>
            ...
            <div className="flex flex-wrap gap-1.5">
                {activePerms.length === 0 ? (
                    ...
                    <span key={p.key} className="text-xs font-semibold px-2 py-1 rounded-lg bg-gray-100 text-slate-700">
                        {p.label}
                    </span>
                ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex">
            ...
            <div className="relative ml-auto w-full max-w-lg bg-white h-full shadow-2xl flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                    <h2 className="text-lg font-bold text-slate-900">{role ? 'Edit Role' : 'Create Role'}</h2>
                    <button onClick={onClose} className="p-2 rounded-xl hover:bg-gray-100 transition-colors"><XCircleIcon className="w-5 h-5 text-slate-400" /></button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-2.5 border border-slate-100 rounded-xl outline-none focus:border-blue-500 text-sm"
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows={2}
                            className="w-full px-4 py-2.5 border border-slate-100 rounded-xl outline-none focus:border-blue-500 text-sm resize-none"
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3">
                            <input type="color" value={form.color} onChange={e => setForm(f => ({ ...f, color: e.target.value }))}
                                className="w-10 h-10 rounded-lg cursor-pointer border border-slate-100 p-0.5" />
                            <span className="text-sm font-mono text-slate-500">{form.color}</span>
                            <span className="text-xs font-bold px-3 py-1.5 rounded-full text-slate-900" style={{ background: form.color }}>Preview</span>
                        </div>
                    </div>
                    {/* Permissions */}
                    <div>
                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Permissions</label>
                        <div className="space-y-2">
                            {PERMISSIONS.map(perm => (
                                <label key={perm.key} className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 cursor-pointer hover:bg-gray-50 transition-colors">
                                    <input
                                        type="checkbox"
                ...
                <div className="flex gap-3 px-6 py-4 border-t border-slate-100">
                    <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-100 text-slate-500 text-sm font-semibold hover:bg-gray-50 transition-colors">Cancel</button>
                    <button onClick={handleSubmit} disabled={saving}
                        className="flex-1 py-2.5 rounded-xl bg-blue-600 text-slate-900 text-sm font-bold hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-60">
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-8">
                <h2 className="text-xl font-bold text-slate-900 mb-1">Create Staff Member</h2>
                <p className="text-sm text-slate-400 mb-6">A new user account will be created and manually assigned a role.</p>
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">First Name</label>
                            <input value={form.first_name} onChange={e => setForm(f => ({ ...f, first_name: e.target.value }))}
                                className="w-full px-4 py-2.5 border border-slate-100 rounded-xl outline-none focus:border-blue-500 text-sm" />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">Last Name</label>
                            <input value={form.last_name} onChange={e => setForm(f => ({ ...f, last_name: e.target.value }))}
                                className="w-full px-4 py-2.5 border border-slate-100 rounded-xl outline-none focus:border-blue-500 text-sm" />
                        </div>
                    </div>
                    <div>
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">Assign Role *</label>
                        <select value={form.role_id} onChange={e => setForm(f => ({ ...f, role_id: e.target.value }))}
                            className="w-full px-4 py-2.5 border border-slate-100 rounded-xl outline-none focus:border-blue-500 text-sm bg-white">
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8">
                ...
                <p className="text-sm text-slate-400 mb-6">Enter a new password for <span className="font-semibold text-slate-700">{member.user?.email}</span></p>
                <div className="space-y-4">
                    <div>
                ...
                <div className="flex gap-3 mt-6">
                    ...
                    <button onClick={handleSubmit} disabled={saving || password.length < 6}
                        className="flex-1 py-2.5 rounded-xl bg-blue-600 text-slate-900 text-sm font-bold hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-60">
                        {saving && <ArrowPathIcon className="w-4 h-4 animate-spin" />}
    ...
    return (
        <div>
            ...
            <div className="flex items-center justify-between mb-6">
                <p className="text-sm text-slate-400">{roles.length} role{roles.length !== 1 ? 's' : ''} defined</p>
                <div className="flex gap-2">
                    <button onClick={handleSeed} className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-50">
                        <ArrowPathIcon className="w-4 h-4" /> Seed Defaults
                    </button>
                    <button onClick={() => { setEditingRole(null); setShowDrawer(true) }}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 shadow-sm">
        ...
        <div>
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setShowCreate(true)}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 shadow-sm transition-colors">
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-gray-50 border-b border-slate-100">
                                <th className="px-5 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Staff Member</th>
                                <th className="px-5 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Role</th>
                                <th className="px-5 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Status</th>
                                <th className="px-5 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Invited By</th>
                            ...
                            {staff.length === 0 ? (
                                ...
                                <tr key={member.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-slate-900 text-sm font-bold flex-shrink-0">
                                    ...
                                    <td className="px-5 py-4">
                                        <select
                                            ...
                                            className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-slate-100 bg-white outline-none focus:border-blue-500 cursor-pointer"
                                        >
                                            {roles.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                                        </select>
                                    </td>
                                    <td className="px-5 py-4">
                                        <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full ${member.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-slate-400'}`}>
                                            <span className={`w-1.5 h-1.5 rounded-full ${member.is_active ? 'bg-emerald-500' : 'bg-gray-400'}`} />
                                            {member.is_active ? 'Active' : 'Inactive'}
                                        </span>
                                    </td>
                                    <td className="px-5 py-4 text-xs text-slate-400">{member.invited_by_email || '—'}</td>
                                    <td className="px-5 py-4">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <button onClick={() => setResetMemberModal(member)}
                                                ...
                                                className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-indigo-50 transition-colors">
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900" style={{ fontFamily: "'Outfit', sans-serif" }}>Roles & Permissions</h1>
                    <p className="text-sm text-slate-500 mt-1">Manage staff access levels and permissions across the admin portal.</p>
                </div>
            </div>
