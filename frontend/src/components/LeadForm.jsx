...
export default function LeadForm() {
    ...
    // const { data: models } = useData('data_models.json') // REMOVED: Now fetching dynamically
    ...
    // Vehicle type classification (Simplified for brevity, same logic applies)
    const getVehicleType = (modelName) => 'sedan'
    // ... (Keeping logic simple for this replacement, ideally imports utility)
    ...
    const handleSubmit = async (e) => {
        ...
        // Resolve IDs to Names for storage (or store IDs if backend changed, but req asked for names typically)
        const makeObj = makes.find(m => m.makeID === parseInt(selectedMake))
        // modelID in json, but we store modelName in selectedModel for simplicity
        // const modelObj = models.find(m => m.modelID === parseInt(selectedModel)) 
        // if (!modelObj && selectedModel) { 
        //      Logic Check: filteredModels uses modelName usually in value, let's fix the select value below first
        // }
        const partObj = parts.find(p => p.partID === parseInt(selectedPart))

        const payload = {
            make: makeObj ? makeObj.makeName : 'Unknown',
            model: selectedModel, // We will store the name in value for simplicity or find name by ID
    ...
    if (isSuccess) {
        return (
            <div className="w-full max-w-sm mx-auto font-sans bg-dark-900/95 backdrop-blur-md p-8 rounded-xl border border-white/10 shadow-2xl text-center flex flex-col items-center justify-center min-h-[400px]">
                ...
                <button
                    onClick={handleReset}
                    className="text-orange-500 font-bold hover:text-orange-400 transition-colors uppercase tracking-wide text-sm border-b border-transparent hover:border-orange-500"
                >
    ...
    return (
        <div className="w-full max-w-sm mx-auto font-sans">
            {/* Header */}
            <div className="bg-gradient-to-r from-orange-400 to-orange-500 rounded-t-xl p-3 text-center shadow-lg">
            ...
            {/* Form Body */}
            <form onSubmit={handleSubmit} className="bg-dark-900/95 backdrop-blur-md p-5 rounded-b-xl border border-white/10 shadow-2xl space-y-3">
                ...
                <div className="space-y-1">
                    ...
                    >
                        ...
                        {/* Filtered by makeID (string parsed to int) */}
                ...
                {/* Security Code */}
                <div className="bg-white/5 p-3 rounded-lg border border-white/10 flex items-center justify-between gap-3">
                ...
                <div className="space-y-2">
                    ...
                    <button
                        type="submit"
                        disabled={submitting}
                        className="w-full bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-400 hover:to-orange-500 text-white font-black text-sm uppercase py-3 rounded-lg shadow-lg hover:shadow-orange-500/20 transition-all transform active:scale-95 disabled:opacity-50 disabled:transform-none"
                    >
