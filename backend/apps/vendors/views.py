    ...
    def update(self, request, *args, **kwargs):
        ...
        if not was_active and vendor.is_active and vendor.email:
            ...
            if not existing_user:
                ...
                # Log activation (DO NOT log password in production)
                logger.info(f"Vendor activated: {vendor.name} (User: {username}, Email: {vendor.email})")
                # Password is returned in API response for admin to share with vendor
                ...
                user = existing_user
                logger.info(f"User already exists for {vendor.email}")
            ...
            if not hasattr(user, 'vendor_profile'):
                ...
                logger.info(f"VendorProfile created for user {user.username} and vendor {vendor.name}")
            else:
                logger.info(f"User {user.username} already has a vendor profile")
