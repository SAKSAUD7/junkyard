...
# Source directory for vendor logos
VENDOR_LOGOS_DIR = r"c:\Users\saksa\Downloads\Telegram Desktop\ALL_IMAGES\ALL_IMAGES\account-logo"
...
def upload_file(blob_service_client, local_file_path, blob_name):
    ...
    try:
        ...
        with open(local_file_path, "rb") as data:
            ...
            elif local_file_path.endswith('.gif'):
...
def main():
    print(f"[*] Starting Vendor Logo Upload to Azure Blob Storage")
    print(f"   Source: {VENDOR_LOGOS_DIR}")
    print(f"   Container: {CONTAINER_NAME}/vendors/")
    ...
    try:
        ...
        if not container_client.exists():
            ...
            print(f"   [OK] Container exists: {CONTAINER_NAME}")
    ...
    for file in os.listdir(VENDOR_LOGOS_DIR):
        if file.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
            ...
            # Upload to vendors/ subdirectory in the media container
            blob_name = f"vendors/{file}"
            files_to_upload.append((local_path, blob_name))

    total_files = len(files_to_upload)
    print(f"\n   [INFO] Found {total_files} vendor logo files to upload")
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for i, future in enumerate(as_completed(futures), 1):
            ...
            if i % 100 == 0 or i == total_files:
                print(f"   Progress: {i}/{total_files} ({(i/total_files)*100:.1f}%) - OK: {success_count} | FAIL: {failure_count}")
    ...
    if failures:
        print(f"\n   Failed uploads:")
        for blob_name, error in failures[:10]:  # Show first 10 failures
