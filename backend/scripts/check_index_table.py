import psycopg2

# Azure Connection
DB_HOST = "junk.postgres.database.azure.com"
DB_USER = "junkyard_admin"
DB_PASSWORD = "saksaud@7411"
DB_NAME = "junkyard"

try:
    conn = psycopg2.connect(dbname=DB_NAME, user=DB_USER, password=DB_PASSWORD, host=DB_HOST, sslmode="require")
    cur = conn.cursor()
    
    print("\n🔍 Checking for 'hollander_index' and 'hollander_hollanderindex':")
    
    tables_to_check = ['hollander_index', 'hollander_hollanderindex', 'hollander_interchange']
    
    for t in tables_to_check:
        cur.execute(f"SELECT count(*) FROM information_schema.tables WHERE table_name = '{t}'")
        exists = cur.fetchone()[0] > 0
        status = "✅ EXISTS" if exists else "❌ MISSING"
        print(f"{t:<30}: {status}")
        
        if exists:
            cur.execute(f"SELECT table_schema FROM information_schema.tables WHERE table_name = '{t}'")
            schema = cur.fetchone()[0]
            print(f"   -> Schema: {schema}")
            
            # Count rows
            try:
                cur.execute(f'SELECT count(*) FROM "{schema}"."{t}"')
                count = cur.fetchone()[0]
                print(f"   -> Rows: {count:,}")
            except Exception as e:
                print(f"   -> Error counting rows: {e}")

    conn.close()
except Exception as e:
    print(f"Error: {e}")
